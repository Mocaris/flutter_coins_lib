...
class TransactionBuilder {
  ...
  TransactionBuilder({NetworkType? network, this.maximumFeeRate = 2500}) {
  ...
  bool _overMaximumFees(int bytes) {
    var incoming = _inputs.fold<int>(0, (cur, acc) => cur + (acc.value ?? 0));
    var outgoing = _tx.outs.fold<int>(0, (cur, acc) => cur + (acc.value ?? 0));
